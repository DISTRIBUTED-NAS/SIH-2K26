...
export const EditMeasurementRecordModal = ({ isOpen, onClose, onUpdate, record }) => {
  ...
  useEffect(() => {
    if (record) {
      setStandardValue(record.standardValue != null ? record.standardValue.toString() : '');
      setObservedValue(record.observedValue != null ? record.observedValue.toString() : '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to update measurement record');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800">Edit Measurement Reading</h3>
              <p className="text-xs text-slate-500">Test Point #{record.testPoint}</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {preview && (
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-bold text-blue-900 uppercase">Recalculation Preview</span>
                <span className="text-[10px] text-blue-600 italic">Backend will verify and persist</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-xs text-blue-700">Recalculated Error:</span>
...
export default EditMeasurementRecordModal;
